...
// Mixpanel SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = MixpanelOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(MixpanelOmni::structfix($now, Struct::undef()))
                        !== json_encode(MixpanelOmni::structfix($val, Struct::undef()))) {
