...
class FtCtrl extends MixpanelControl
{
...
class FtClient
{
    ...
    private MixpanelUtility $utility;

    public function __construct(MixpanelUtility $utility, array $options)
    {
    ...
    public function get_utility(): MixpanelUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MixpanelContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MixpanelContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MixpanelContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MixpanelConfig::make_feature($name);
    ...
    private static function buildUrl(MixpanelSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MixpanelError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MixpanelResponse($fetched);
        }
    ...
    private function fail(MixpanelContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MixpanelContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
