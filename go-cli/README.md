...
boru-driven command-line client **and** interactive REPL for the Mixpanel
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mixpanel-cli)
make build
...
MIXPANEL_BASE=https://api.example.com ./mixpanel-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mixpanel-cli
   ```
...
Run with no arguments to open a REPL (prompt `mixpanel>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `engage`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mixpanel-cli`. |
...
(`./mixpanel-cli <expr>`) and interactively (the REPL), and expressions compose the
