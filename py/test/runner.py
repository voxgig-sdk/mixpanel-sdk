...
class MixpanelTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MixpanelTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MixpanelTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MixpanelTestRunner.getenv("MIXPANEL_TEST_LIVE")
        override = MixpanelTestRunner.getenv("MIXPANEL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MixpanelTestRunner.getenv(key)
        ...
        explain = MixpanelTestRunner.getenv("MIXPANEL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MIXPANEL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MixpanelTestRunner._test_control is not None:
            return MixpanelTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MixpanelTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MixpanelTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MixpanelTestRunner.load_test_control()
...
def load_env_local():
    MixpanelTestRunner.load_env_local()


def env_override(m):
    return MixpanelTestRunner.env_override(m)


def entity_data(v):
    return MixpanelTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MixpanelTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MixpanelTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MixpanelTestRunner.load_test_control()


def live_client_options():
    return MixpanelTestRunner.live_client_options()


def live_delay_ms():
    return MixpanelTestRunner.live_delay_ms()
