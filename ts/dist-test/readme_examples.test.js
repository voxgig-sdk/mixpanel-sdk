...
const TEST_SEED = { "entity": { "engage": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+MixpanelSDK\b/.test(code) ||
        /\bMixpanelSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+MixpanelSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/MixpanelSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
