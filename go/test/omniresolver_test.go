...
import (
	...
	omni "github.com/voxgig-sdk/mixpanel-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MixpanelError); is {
				out := map[string]any{
					"name":    "MixpanelError",
...
func omniCtx(arg any, client *sdk.MixpanelSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MixpanelSDK); is {
			client = live
