...
{
    private static function utility(): MixpanelUtility
    {
        return new MixpanelUtility();
    }
    ...
    private static function ctx(array $over = []): MixpanelContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new MixpanelContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new MixpanelOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof MixpanelError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new MixpanelOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = MixpanelMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new MixpanelOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = MixpanelMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new MixpanelOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = MixpanelMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MixpanelSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = MixpanelMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new MixpanelResponse([]),
            'result' => new MixpanelResult([]),
        ]);
        [, $err] = MixpanelMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new MixpanelSpec([]),
            'response' => null,
            'result' => new MixpanelResult([]),
        ]);
        [, $err] = MixpanelMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new MixpanelSpec(['step' => 's']),
            'response' => new MixpanelResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new MixpanelResult([]),
        ]);
        [, $err] = MixpanelMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new MixpanelSpec(['step' => 's']),
            'response' => new MixpanelResponse(self::resp(200, ['v' => 1])),
            'result' => new MixpanelResult([]),
        ]);
        [, $err] = MixpanelMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new MixpanelSpec(['step' => 's']),
            'response' => new MixpanelResponse(self::resp(200, ['v' => 2])),
            'result' => new MixpanelResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        MixpanelMakeResponse::call($ctx);
    ...
    {
        $preset = new MixpanelResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new MixpanelSpec([]),
            'response' => new MixpanelResponse([]),
            'result' => new MixpanelResult([]),
        ...
        [$got, $err] = MixpanelMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new MixpanelResult([])]);
        [, $err] = MixpanelMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new MixpanelSpec([]), 'result' => null]);
        [, $err] = MixpanelMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MixpanelSpec(['step' => 's']),
            'result' => new MixpanelResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new MixpanelOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MixpanelMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MixpanelSpec(['step' => 's']),
            'result' => new MixpanelResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new MixpanelOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MixpanelMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MixpanelResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new MixpanelSpec([]),
            'result' => new MixpanelResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = MixpanelMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MixpanelMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new MixpanelError('boom', 'boom');
        $utility->fetcher = function (MixpanelContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MixpanelSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = MixpanelMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (MixpanelContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = MixpanelMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(MixpanelResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (MixpanelContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MixpanelSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = MixpanelMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new MixpanelResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new MixpanelSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = MixpanelMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = MixpanelMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = MixpanelMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new MixpanelResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, MixpanelDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new MixpanelResult(['ok' => false])]);
        $this->expectException(MixpanelError::class);
        MixpanelDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new MixpanelResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', MixpanelMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        MixpanelMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new MixpanelBaseFeature();
        $b = new MixpanelBaseFeature();
        MixpanelFeatureAdd::call($ctx, $a);
        MixpanelFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): MixpanelBaseFeature
    {
        $f = new MixpanelBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        MixpanelFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): MixpanelContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new MixpanelSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MixpanelPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new MixpanelResponse(['headers' => null]),
            'result' => new MixpanelResult([]),
        ]);
        MixpanelResultHeaders::call($ctx);
