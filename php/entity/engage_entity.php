...
class EngageEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EngageEntity($this->_client, $opts);
    }

    /**
     * @param Engage|array $args Engage data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MixpanelHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Engage|array The current Engage data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Engage fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MixpanelHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Engage fields).
     */
     ...
     * Create a new Engage.
     *
     * @param EngageCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed EngageCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Engage|array The created Engage as an assoc-array at the
     *   SDK boundary; throws MixpanelError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = MixpanelHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
