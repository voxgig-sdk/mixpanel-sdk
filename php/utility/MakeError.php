...
class MixpanelMakeError
{
    public static function call(?MixpanelContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MixpanelContext([], null);
        }
        $op = $ctx->op ?? new MixpanelOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MixpanelResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MixpanelError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MixpanelError('', $msg, $ctx);
