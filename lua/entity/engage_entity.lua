...
local EngageEntity = {}
EngageEntity.__index = EngageEntity


function EngageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EngageEntity)
  self._name = "engage"
  self._client = client
...
function EngageEntity:get_name()
  return self._name
end


function EngageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EngageEntity.new(self._client, opts)
end
...
function EngageEntity:mark_deleted()
  self._deleted = true
end


function EngageEntity:deleted()
  return true == self._deleted
end


function EngageEntity:data_set(args)
  if args ~= nil then
...
function EngageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EngageEntity:match_set(args)
  if args ~= nil then
...
function EngageEntity:match_get()
...
function EngageEntity:stream(action, args, callopts)
...
---@param reqdata EngageCreateData
---@param ctrl? table
---@return Engage
---@return string? err
function EngageEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function EngageEntity:_run_op(ctx, post_done)
  local utility = self._utility
