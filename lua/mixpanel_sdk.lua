...
local MixpanelSDK = {}
MixpanelSDK.__index = MixpanelSDK
...
MixpanelSDK._make_feature = _make_feature


function MixpanelSDK.new(options)
  local self = setmetatable({}, MixpanelSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MixpanelSDK:options_map()
...
function MixpanelSDK:get_utility()
  return Utility.copy(self._utility)
end


function MixpanelSDK:get_root_ctx()
  return self._rootctx
end


function MixpanelSDK:prepare(fetchargs)
...
function MixpanelSDK:direct(fetchargs)
...
-- by passing it.
function MixpanelSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MixpanelSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MixpanelSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:Engage():list() / client:Engage():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function MixpanelSDK:Engage(data)
  local EntityMod = require("entity.engage_entity")
  if data == nil then
    if self._engage == nil then
      self._engage = EntityMod.new(self, nil)
    end
    return self._engage
  end
...
function MixpanelSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MixpanelSDK.new(sdkopts)
  sdk.mode = "test"
