...
[MCP](https://modelcontextprotocol.io) server exposing the Mixpanel SDK as
two agent tools — `mixpanel_list` and `mixpanel_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mixpanel-mcp)
make build

# 2. Provide credentials via the environment
export MIXPANEL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user mixpanel \
  -- /absolute/path/to/mixpanel-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./mixpanel-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// mixpanel_list: first page of records
{ "entity": "engage" }
{ "entity": "engage", "query": { } }

// mixpanel_load: one record by id
{ "entity": "engage", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mixpanel-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mixpanel \
     -- "$PWD"/dist/*/mixpanel-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mixpanel_list` and `mixpanel_load` tools now appear
   in new sessions. Ask the agent to *"list engage using mixpanel"*
   and it calls `mixpanel_list` with `{"entity":"engage"}`.
...
```sh
export MIXPANEL_APIKEY=sk_live_xxx            # API key
export MIXPANEL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `mixpanel_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mixpanel_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MIXPANEL_APIKEY` | API key sent with every request. |
| `MIXPANEL_BASE` | Optional override of the API base URL. |
...
```sh
./mixpanel-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mixpanel_load","arguments":{"entity":"engage","query":{"id":1}}}}'
```
